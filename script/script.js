...
const sortByPopularityCheckbox = document.querySelector('#cbx-46');
const searchByTagsCheckbox = document.querySelector('#cbx-46-2');
let players = [];
...
function trouverJoueur(recherche, players) {
    ...
    if (sortByPopularityCheckbox.checked) {
        joueursFiltres = joueursFiltres.sort((a, b) => a.distance - b.distance || b.player.popularite - a.player.popularite);
    }
    else {
        joueursFiltres = joueursFiltres.sort((a, b) => a.distance - b.distance || a.player.nom.localeCompare(b.player.nom));
    }
